...
case class Boom(x: Double, y: Double) extends Event {
    override def appliesTo(e: Entity): Boolean = e match {
        case ArcadeBaloon(_, x1, y1, _) => {
            math.sqrt((x-x1)*(x-x1)+(y-y1)*(y-y1)) <= ScreenDimensions.baloonBurstDistance /*-- TODO: Boom applies to an arcade baloon if euclidean distance between (x1, y1) and (x,y) is less than ScreenDimensions.baloonBurstDistance --*/
        }
        case _ => false
    }
}
...
case class ArcadeGameManager(val gamePoints: Int) extends Entity {
    ...
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) = e match {
        case Tick(_) => { // TODO
            ...
            if (rd < 0.02) {
                val fory = 500 + Random.nextInt((600-500)+1)
                (List(this, ArcadeBaloon(id = global.numEntities+1, x=20, y= fory, driftX = 10.0 * Random.nextDouble)), List[Event]())
            } else if (rd > .98) {
                (List(this, ArcadeBaloon(id = global.numEntities + 1, x = ScreenDimensions.width - 20, y = 500 + Random.nextInt((600 - 500) + 1), driftX = -10.0 * Random.nextDouble)), List[Event]())
            } else {
                (List(this), List[Event]())
            }
        }
        case BaloonEscaped => {
            val newGP = gamePoints + ScreenDimensions.pointsBaloonEscaped
            (List(ArcadeGameManager(newGP)), List[Event]())
        }
        case BaloonBurst => {
            val newGP = gamePoints + ScreenDimensions.pointsBaloonBurst
            (List(ArcadeGameManager(newGP)), List[Event]())
        }
        case Stop => {
            (List(), List())
