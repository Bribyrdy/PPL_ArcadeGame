...
class MainEventLoop {
    ...
    */
    def processOneEvent (l: List[Entity]) (ev: Event): (List[Entity], List[Event]) = {
        ...
        val (lApplies, lDoesNotApply) = partition(l)(ev)
        val (entities: List[Entity], events:List[Event]) = lApplies.foldLeft[(List[Entity], List[Event])] (List(),List()) {
            case ((ent,env), current) => {
                val (newEnt, newEnv) = current.handleEvent(this)(ev)
                (ent ++ newEnt, env++newEnv)
            }
        }
        (lDoesNotApply ++ entities, events)
    }
   ...
   */
    final def performOneTransition: (List[Entity], List[Event]) = {
        ...
        val (newEntities: List[Entity],newEvents: List[Event]) = events.foldLeft[(List[Entity], List[Event])] (entities,List()){
            case ((trEnt,freshEve), ev) => {
                val (trEntities, freshEvents) = processOneEvent(trEnt)(ev)
                (trEntities, freshEve ++ freshEvents)
            }
        }
        (newEntities, newEvents)
    }
